...
try:
    ...
    print("⚠️  TensorFlow not available - will use simple feature extraction")
...
class Config:
    # Input paths
    BASE_PATH = Path('c:/Users/Yashaswi/Desktop/AgrigenAI/AgriGenAI_Dataset')
    PLANTVILLAGE_PATH = BASE_PATH / 'PlantVillage/images'
    LABORO_PATH = BASE_PATH / 'Laboro'
    
    # Output paths
    OUTPUT_PATH = Path('c:/Users/Yashaswi/Desktop/AgrigenAI/AgrigenAI_Output')
...
if TENSORFLOW_AVAILABLE:
    try:
        ...
        print(f"⚠️  ResNet50 failed: {e}, using simple features")
        TENSORFLOW_AVAILABLE = False
else:
    print("ℹ️  Using simple feature extraction (color histograms + shape)")
...
if TENSORFLOW_AVAILABLE:
    ...
    print("   (Using simple features - faster extraction)")

def extract_simple_features(img_path):
    """Extract simple color and shape features without TensorFlow"""
    try:
        ...
        hist = np.histogram(img_array.mean(axis=2), bins=16)[0]
        features = np.concatenate([features[:2048], hist.astype(np.float32)])
...
for idx, row in df.iterrows():
    ...
    try:
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            features = extract_simple_features(row['image_path'])
...
print(f"      cd c:\\Users\\Yashaswi\\Desktop\\AgrigenAI\\AgrigenAI_Code")
...
if not TENSORFLOW_AVAILABLE:
    print(f"   TensorFlow was not available, so simple features were used.")
    print(f"   Models will still work! For better accuracy, install TensorFlow:")
    print(f"   pip install tensorflow")
else:
    print(f"   Full ResNet50 features used for best accuracy!")

print("\n" + "="*70)
print("🎉 SUCCESS! Models trained with PlantVillage + Laboro data!")
print("="*70)antVillage: {len(plantvillage_data)}")
print(f"   Laboro: {len(laboro_data)}")
