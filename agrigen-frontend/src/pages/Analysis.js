...
import "./Analysis.css";
import FloatingChatbot from "../components/FloatingChatbot"; // ← ADD THIS LINE

const API_BASE_URL = "http://localhost:5000";
// APIs for new features
const GEMINI_API_KEY = "AIzaSyCVp9mXiOuM7n5soqtvp2SVenloUo14XHE";
...
function Analysis() {
  ...
  // New states for location and validation
  ...
  const validateTomatoImage = async (imageFile) => {
    try {
      ...
      return new Promise((resolve) => {
        reader.onloadend = async () => {
          try {
                  ...
                  contents: [
                    {
                      parts: [
                        {
                          text: "Is this a tomato plant? Answer: 'YES' if tomato, 'NO' if not.",
                        },
                        {
                          inline_data: {
            ...
            if (!response.ok) {
              resolve({ isValid: true, message: "Validation skipped" });
              return;
            }
            ...
            if (result.includes("YES")) {
              resolve({ isValid: true, message: "Valid tomato plant" });
            } else {
              resolve({
                isValid: false,
                message:
                  "Not a tomato plant. Please upload tomato plant image.",
              });
            }
          } catch (err) {
            console.error(err);
  ...
  // Location detection - Road, Area, City, State format
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // Get detailed address with zoom=18 and accept-language=en
          const response = await fetch(
            `https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&zoom=18&addressdetails=1&accept-language=en`,
            {
              headers: {
                "User-Agent": "AgriGenAI/1.0",
                "Accept-Language": "en",
              },
            }
          );
          ...
          if (response.ok) {
            ...
            // Build location: Road, Area, City, State
            const parts = [];
            ...
            // Area/Neighbourhood (try multiple fields)
            const area =
              addr.neighbourhood ||
              addr.suburb ||
              addr.quarter ||
            ...
            // Join with ", " - Result: "Bogadi Main Rd, Vijayanagar, Mysuru, Karnataka"
      ...
      (err) => {
        ...
        if (err.code === 3)
          errorMsg = "Location timeout. Please enter manually.";
  ...
  const onDrop = async (acceptedFiles) => {
    ...
    if (file) {
      ...
      try {
        const validation = await validateTomatoImage(file);
        ...
        setImage(file);
        setImagePreview(URL.createObjectURL(file));
        toast.warning("Validation skipped");
      } finally {
        setValidatingImage(false);
      }
    }
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    multiple: false,
    disabled: selectedCrop !== "tomato" || validatingImage,
  });
        ...
        <div className="upload-section">
          ...
          <div
            ...
            className={`dropzone ${isDragActive ? "active" : ""} ${
              selectedCrop !== "tomato" || validatingImage ? "disabled" : ""
            }`}
            style={{
              cursor:
                selectedCrop !== "tomato" || validatingImage
                  ? "not-allowed"
                  : "pointer",
            }}
          >
            ...
            {validatingImage ? (
              <div className="dropzone-content">
                ...
                <small>Checking if this is a tomato plant...</small>
              </div>
            ) : imagePreview ? (
              <div>
                <img
              ...
              <div className="dropzone-content">
                ...
                <p>
                  {selectedCrop === "tomato"
                    ? "Drop your tomato plant image here, or click to select"
                    : "Please select Tomato to upload images"}
                </p>
                ...
                >
                  ⚠️ Only clear tomato plant images will be accepted
                </small>
              </div>
            )}
          </div>
          ...
          <div className="location-input">
            ...
            >
              ...
              >
                {gettingLocation ? (
                  <>
                    <Loader size={16} className="spinner" /> Getting Location...
                  </>
                ) : (
                  <>
              ...
              {!gettingLocation && location !== "Mysuru,IN" && (
                <span
                  style={{
                    color: "#10b981",
                    fontSize: "0.9rem",
            ...
            <input
              ...
              placeholder="Or enter location manually (e.g., Street, Area, City, State, PIN)"
              disabled={gettingLocation}
              style={{ opacity: gettingLocation ? 0.6 : 1 }}
            />
            <small
              style={{
                color: "#666",
            ...
            >
              💡 Click "Use Current Location" for full address or type manually
            </small>
          </div>
